...
class WES_State(object):
    ...
    def getValidMoves(self, move, moveMade):
        ...
        if self.blackToMove and distance <= 2 and self.board[startRow][startCol] == 'bW':
            valid_ = self.getValidWolf(move, distance)
        elif not self.blackToMove and distance == 1 and self.board[startRow][startCol] == 'wS':
    ...
    def getValidWolf(self, move, distance):
        ...
        endCol = move.endCol
        if distance == 2:
            if self.board[endRow][endCol] == 'wS' and self.board[(startRow + endRow) // 2][(startCol + endCol) // 2] == '--':
                return True
            else:
                return False
        else:
    ...
    def checkWinning(self):
        ...
        for r in range(len(self.board)):
            for c in range(len(self.board[r])):
                if self.board[r][c] == 'wS':
                    sheep_num += 1
                elif self.board[r][c] == 'bW':
